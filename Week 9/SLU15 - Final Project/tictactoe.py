...
class TicTacToe():
    ...
    @staticmethod
    def get_row_col_from_pixels(pixel_coord: Tuple[int, int]) -> Tuple[int, int]:
        """
        ...
        # compute row
        row_pixel=pixel_coord[1]

        if row_pixel < 310:
            row = 0
        elif row_pixel >310 and row_pixel < 530:
            row = 1
        elif row_pixel > 530:
            row = 2
        
        # compute col
        col_pixel=pixel_coord[0]

        if col_pixel < 310:
            col = 0
        elif col_pixel >310 and col_pixel < 530:
            col = 1
        elif col_pixel > 530:
            col = 2
    ...
    def check_winner(self) -> int:
        ...
        for row_state in self.state:
            
            if row_state == [1,1,1]:
                return CROSS

            elif row_state == [2,2,2]:
                return BALL
        ...
        for i in range(3):
            ...
            elif state[0][i]==2 and state[1][i]==2 and state[2][i]==2:
                return BALL
        ...
        if (state[0][0] == 1 and state[1][1] == 1 and state[2][2] == 1) or (state[2][0] == 1 and state[1][1] == 1 and state[0][2] == 1):
            return CROSS

        elif state[0][0] == 2 and state[1][1] == 2 and state[2][2] or (state[2][0] == 2 and state[1][1] == 2 and state[0][2] == 2):
            return BALL
    ...
    def get_empty_positions(self) -> List[Tuple[int, int]]:
        ...
        for state_row in self.state:
            ...
            for state_col in state_row:
                print(state_col)
                if state_col == 0:
                    position_tuple=(col,row)
                    empty_pos.append(position_tuple)
                    print(empty_pos)
                col+=1
            col=0
            row+=1
